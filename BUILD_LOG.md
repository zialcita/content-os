...
## 2026-09-14 — Working private text-source workflow

Continued from b63ffbf52f85e4d52905c5ec1d32974ac5578cba. Reviewed/repaired the previously partial content SQL and implemented new additive migration003 (no committed001/002SQL edits), explicit content/worker privileges and combined audits, private local filesystem store, typed upload/status/version/preview API, nonpaid fenced ingestion worker/CLI, and a same-origin development workbench. Fixed partial SQL CASE/trigger-record errors before adoption. Added symlink/hardlink/FIFO and root/namespace permission checks, exact transfer bounds, source hash/MIME/UTF8 checks, immutable typed versions, current-authority checks and cancellation/purge separation.

Actual Chromium test completed sign-in via signed fake IdP, workspace/brand selection, real-file transfer, queued status, separate actual worker validation and source text/version display. Playwright network bridge calls real ASGI/PostgreSQL and asserts server303 response; bridge navigation adapts synthetic redirect hops. No fake completion or paid-provider success is returned. Browser screenshot reviewed; test users are labeled fixtures.

Final content/browser suite22 passed. Existing identity71, foundation44, runtime59, contract43 and isolation5 suites plus Next.js build passed. Identity suite now explicitly exercises its002revision; a separate content test audits current003 head. Backend browser-test dependencies pinned separately. Source/runtime schema/log/hash evidence under docs/evidence/m1/content-core and workflow-checkpoint. Screenshot delivered separately as thread file because text-only integration commits cannot include PNG bytes.

Limits kept explicit: implemented scanner is a real bounded TEXT_POLICY_V1/UTF8 policy, NOT general antivirus. Nontext formats remain BLOCKED. Local private storage is not an S3 acceptance substitute. Full registry/approval/job-budget unification, cloud/media integrations, production IdP/hosting and full M1/R1 acceptance remain open. CI remains excluded; no provider spending, posts, real invitations, production data changes, merge or deployment.
