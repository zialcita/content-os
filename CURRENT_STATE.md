...
Private-source checkpoint2026-09-14: an actual Chromium workflow now signs in with a signed fixture IdP, selects an authorized workspace/brand, transfers real private text bytes, queues a PostgreSQL ingestion job, runs the separate worker, and displays validated text with immutable version/hash lineage. Additive revision003 extends scoped grants/audits without editing001/002. Content/browser suite22 passed; existing identity71/foundation44/runtime59/contracts43/isolation5 regressions and Next.js build pass. Text policy is NOT antivirus certification; PDF/audio/video are blocked for missing scanner/probe, cloud storage and production accounts remain unconfigured. This is a working LOCAL DEVELOPMENT workflow, not full M1/R1 completion. See docs/M1_WORKING_WORKFLOW.md. CI, all spending/publishing/merge/deploy gates remain closed. Prior checkpoint descriptions below are history.

Identity checkpoint2026-09-14: separate development API for OIDC code/PKCE, opaque PostgreSQL sessions, CSRF, workspace/brand permissions and local invitation receipts is reviewed and integrated with additive revision002. Identity tests71 passed with actual PostgreSQL/ASGI and a signed fake IdP. Dependencies are pinned; global migration audits run after each revision and on reruns. See docs/IDENTITY_REVIEW.md and docs/M1_IDENTITY_API.md. This is not live-provider proof or frontend login integration. Full M1 remains OPEN; uploads and CI are excluded. Earlier status paragraphs below are prior checkpoints.
